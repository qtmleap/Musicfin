...
struct AstraSearchView: View {
    ...
    private var browse: some View {
        Group {
            ...
            Section("ライブラリから探す") {
                NavigationLink {
                    AstraLibraryCollectionView(kind: .artists)
                } label: {
                ...
                ForEach(items) { item in
                    if item.type == .audio {
                        Button {
                            ...
                            AstraTrackRow(track: item, showsArtwork: true, isPlaying: player.currentItem?.id == item.id)
                        }
                        .buttonStyle(.plain)
                    } else if item.type == .musicArtist {
                        NavigationLink {
                            AstraArtistDetailView(artist: item)
                        } label: {
                            AstraArtistRow(artist: item)
                        }
                    } else {
                        NavigationLink {
                            AstraAlbumDetailView(album: item)
                        } label: {
                            HStack(spacing: 12) {
                                ArtworkView(item: item, size: 52, cornerRadius: 4)
                                    .accessibilityHidden(true)
                                VStack(alignment: .leading, spacing: 4) {
                                    Text(item.displayName).foregroundStyle(.primary)
                                    if let artist = item.albumArtist ?? item.displayArtist {
    ...
    private var searchStatus: some View {
        if !term.isEmpty {
            if isSearching {
                ...
                AstraLibraryLoadError(message: errorMessage) { retryGeneration += 1 }
...
#Preview {
    NavigationStack { AstraSearchView() }
